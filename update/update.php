...
if (is_file($installFile)) {
    ...
    switch ($step) {
        ...
        if ($_POST) {
            ...
            curl_setopt($curl_handle, CURLOPT_URL, 'http://api.cloudnet.com.kh/v1/license/');
        ...
        if ($_POST) {
            ...
            $curl_handle = curl_init();
            curl_setopt($curl_handle, CURLOPT_URL, 'http://api.cloudnet.com.kh/v1/dbtables/');
            curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($curl_handle, CURLOPT_POST, 1);
            curl_setopt($curl_handle, CURLOPT_POSTFIELDS, array(
                'username' => $_POST["username"],
            ...
            if ($object->status == 'success') {
                if(isset($object->database)) {
                    ...
                    if ($database->update_db($dbdata, $_POST['username'], $_POST['code']) == false) {
                        ...
                        $finished = TRUE;
                        if(!@unlink('../ERP_POS')){
                            echo "<div class='alert alert-warning'><i class='icon-warning'></i> Please remove the ERP_POS file from the main folder in order to lock the update tool.</div>";
                        }
                    }
