...
class Pay extends MY_Shop_Controller
{
        ...
        if ($inv = $this->pay_model->getSaleByID($id)) {
            ...
            if ($paypal->active && (($inv->grand_total - $inv->paid) > 0)) {
                ...
                $data = array(
                    ...
                    'business' => (DEMO ? 'saleem-facilitator@cloudnet.com.kh' : $paypal->account_email),
    ...
    function pipn() {
        ...
        $paypal = $this->pay_model->getPaypalSettings();
        $this->erp->log_payment('INFO', 'Paypal IPN called');
        $ipnstatus = FALSE;
        ...
        if (!$fp) {
            ...
            $this->erp->log_payment('ERROR', 'Paypal Payment Failed (IPN HTTP ERROR)', $errstr);
            ...
            while (!feof($fp)) {
                ...
                if (stripos($res, "VERIFIED") !== false) {
                    $this->erp->log_payment('INFO', 'Paypal IPN - VERIFIED');
                    ...
                    if (($_POST['payment_status'] == 'Completed' || $_POST['payment_status'] == 'Processed' || $_POST['payment_status'] == 'Pending') && ($_POST['business'] == $paypal->account_email || $_POST['business'] == 'saleem-facilitator@cloudnet.com.kh')) {
                        ...
                        if ($inv = $this->pay_model->getSaleByID($invoice_no)) {
                            ...
                            if ($this->pay_model->addPayment($payment)) {
                                ...
                                $this->erp->log_payment('SUCCESS', 'Payment has been made for Sale Reference #' . $_POST['item_name'] . ' via Paypal (' . $_POST['txn_id'] . ').', json_encode($_POST));
                                try {
                                    $this->erp->send_email($paypal->account_email, 'Payment made for sale '.$inv->reference_no, $message);
                                } catch (Exception $e) {
                                    $this->erp->log_payment('Email Notification Failed: ' . $e->getMessage());
                                }
                        ...
                        $this->erp->log_payment('ERROR', 'Payment failed for Sale Reference #' . $reference . ' via Paypal (' . $_POST['txn_id'] . ').', json_encode($_POST));
                        $this->session->set_flashdata('error', lang('payment_failed'));

                    }
                } else if (stripos($res, "INVALID") !== false) {
                    $this->erp->log_payment('ERROR', 'INVALID response from Paypal. Payment failed via Paypal.', json_encode($_POST));
                    $this->session->set_flashdata('error', lang('payment_failed'));
                }
            }
            fclose($fp);
    ...
    function sipn() {
        ...
        $skrill = $this->pay_model->getSkrillSettings();
        $this->erp->log_payment('INFO', 'Skrill IPN called', json_encode($_POST));
        $ipnstatus = FALSE;
            ...
            if (strtoupper(md5($concatFields)) == $_POST['md5sig'] && $_POST['status'] == 2 && $_POST['pay_to_email'] == $skrill->account_email) {
                ...
                if ($inv = $this->pay_model->getSaleByID($invoice_no)) {
                    ...
                    if ($this->pay_model->addPayment($payment)) {
                        ...
                        $msg = file_get_contents('./themes/' . $this->Settings->theme . '/admin/views/email_templates/payment.html');
                        $message = $this->parser->parse_string($msg, $parse_data);
                        $this->erp->log_payment('SUCCESS', 'Payment has been made for Sale Reference #' . $_POST['item_name'] . ' via Skrill (' . $_POST['mb_transaction_id'] . ').', json_encode($_POST));
                        try {
                            $this->erp->send_email($skrill->account_email, 'Payment made for sale '.$inv->reference_no, $message);
                        } catch (Exception $e) {
                            $this->erp->log_payment('Email Notification Failed: ' . $e->getMessage());
                        }
                        $this->session->set_flashdata('message', lang('payment_added'));
                ...
                $this->erp->log_payment('ERROR', 'Payment failed for via Skrill.', json_encode($_POST));
                $this->session->set_flashdata('error', lang('payment_failed'));
            }
        } else {
