...
class Api_settings extends MY_Controller
{
    ...
    public function delete($id) {
        if (DEMO) {
            $this->erp->send_json(array('error' => 1, 'msg' => lang("disabled_in_demo")));
        }

        if ($this->api_model->deleteApiKey($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("api_key_deleted")));
        } else {
            $this->erp->send_json(array('error' => 1, 'msg' => lang("delete_failed")));
        }
    }
