...
class Pos extends MY_Controller
{
    ...
    public function index($sid = NULL)
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (!$suspend) {
                $p = isset($_POST['amount']) ? sizeof($_POST['amount']) : (isset($_POST['pos_paid_other'])?sizeof($_POST['pos_paid_other']):0);
                $paid = 0;
                for ($r = 0; $r < $p; $r++) {
                    if ((isset($_POST['amount'][$r]) && !empty($_POST['amount'][$r]) && isset($_POST['paid_by'][$r]) && !empty($_POST['paid_by'][$r])) || (isset($_POST['amount_kh'][$r])&& !empty($_POST['amount_kh'][$r]))) {
                        $amount = $this->erp->formatDecimal($_POST['balance_amount'][$r] > 0 ? ($_POST['amount'][$r]+($_POST['amount_kh'][$r]/$_POST['exchange_kh'][$r]) - $_POST['balance_amount'][$r]): ($_POST['amount'][$r]+$_POST['amount_kh'][$r]/$_POST['exchange_kh'][$r]));
                        ...
                        if ($_POST['paid_by'][$r] == 'gift_card') {
                            ...
                            $payment[] = array(
                                ...
                                'pos_paid'     => $_POST['amount'][$r]+($_POST['amount_kh'][$r]/$_POST['exchange_kh'][$r]),
                                'pos_paid_usd'      => $_POST['amount'][$r],
                                'pos_paid_other'     => $_POST['amount_kh'][$r],
                                'pos_paid_other_rate' => $_POST['exchange_kh'][$r],
