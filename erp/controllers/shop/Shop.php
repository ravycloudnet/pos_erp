...
class Shop extends MY_Shop_Controller
{
        ...
        if ($products = $this->shop_model->getProducts($filters)) {
            ...
            foreach($products as &$value) {
                ...
                if ($this->shop_settings->hide_price) {
                    ...
                    $value['price'] = $this->erp->setCustomerGroupPrice($value['price'], $this->customer_group);
                    $value['formated_price'] = $this->erp->convertMoney($value['price']);
                    $value['promo_price'] = $this->erp->isPromo($value) ? $value['promo_price'] : 0;
                    $value['formated_promo_price'] = $this->erp->convertMoney($value['promo_price']);
                    $value['special_price'] = isset($value['special_price']) && !empty($value['special_price']) ? $this->erp->setCustomerGroupPrice($value['special_price'], $this->customer_group) : 0;
                    $value['formated_special_price'] = $this->erp->convertMoney($value['special_price']);
                }
            }
            ...
            $this->erp->send_json(array('filters' => $filters, 'products' => $products, 'pagination' => $pagination, 'info' => $info));
        } else {
            $this->erp->send_json(array('filters' => $filters, 'products' => FALSE, 'pagination' => FALSE, 'info' => FALSE));
        }
    }
    ...
    function orders($id = NULL, $hash = NULL, $pdf = NULL, $buffer_save = NULL) {
        ...
        if ($id && !$pdf) {
            ...
            if ($this->input->get('view')) {
                ...
                if ($buffer_save) {
                    return $this->erp->generate_pdf($html, $name, $buffer_save, $this->data['biller']->invoice_footer);
                } else {
            ...
            if ($guest_checkout || $address = $this->shop_model->getAddressByID($this->input->post('address'))) {
                ...
                foreach ($this->cart->contents() as $item) {
                    ...
                    if ($product_details = $this->shop_model->getProductForCart($item['product_id'])) {
                        $price = $this->erp->setCustomerGroupPrice(($product_details->special_price ? $product_details->special_price : $product_details->price), $this->customer_group);
                        $price = $this->erp->isPromo($product_details) ? $product_details->promo_price : $price;
                        if ($item['option']) {
                ...
                $grand_total = $this->erp->formatDecimal(($total + $total_tax + $shipping), 4);
                ...
                // $this->erp->print_arrays($data, $products, $customer, $address);
    ...
    function address($id = NULL) {
        if (!$this->loggedIn) { $this->erp->send_json(array('status' => 'error', 'message' => lang('please_login'))); }
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (count($user_addresses) >= 6) {
                $this->erp->send_json(array('status' => 'error', 'message' => lang('already_have_max_addresses'), 'level' => 'error'));
            }
            ...
            if ($id) {
                ...
                $this->erp->send_json(array('redirect' => $_SERVER['HTTP_REFERER']));
            } else {
            ...
            $this->erp->send_json(array('status' => 'error', 'message' => validation_errors()));
        } else {
    ...
    function send_message() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            try {
                if ($this->erp->send_email($this->shop_settings->email, $this->input->post('subject'), $this->input->post('message'), $this->input->post('email'), $this->input->post('name'))) {
                    $this->erp->send_json(array('status' => 'Success', 'message' => lang('message_sent')));
                }
                $this->erp->send_json(array('status' => 'error', 'message' => lang('action_failed')));
            } catch (Exception $e) {
                $this->erp->send_json(array('status' => 'error', 'message' => $e->getMessage()));
            }

        } elseif ($this->input->is_ajax_request()) {
            $this->erp->send_json(array('status' => 'Error!', 'message' => validation_errors(), 'level' => 'error'));
        } else {
    ...
    public function order_received($id = null, $hash = null)
    {
        if ($inv = $this->shop_model->getOrder(['id' => $id, 'hash' => $hash])) {
            ...
            try {
                if ($this->erp->send_email(($user ? $user->email : $customer->email), $subject, $message, null, null, $attachment, $cc, $bcc)) {
