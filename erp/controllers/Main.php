...
class Main extends MY_Shop_Controller
{
    ...
    function forgot_password() {
        ...
        if ($this->form_validation->run() == false) {
            $this->erp->send_json(validation_errors());
        } else {

            $identity = $this->ion_auth->where('email', strtolower($this->input->post('email')))->users()->row();
            if (empty($identity)) {
                $this->erp->send_json(lang('forgot_password_email_not_found'));
            }

            $forgotten = $this->ion_auth->forgotten_password($identity->email);
            if ($forgotten) {
                $this->erp->send_json(['status' => 'success', 'message' => $this->ion_auth->messages()]);
            } else {
                $this->erp->send_json(['status' => 'error', 'message' => $this->ion_auth->errors()]);
            }
        }
    }
