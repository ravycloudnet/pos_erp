...
class Tec_cart
{
    ...
    protected function _save_cart() {
        ...
        foreach ($this->_cart_contents as $key => $val) {
            ...
            $this->_cart_contents['cart_total'] += $this->erp->formatDecimal(($val['price'] * $val['qty']), 4);
            $this->_cart_contents['total_item_tax'] += $this->erp->formatDecimal(($val['tax'] * $val['qty']), 4);
            $this->_cart_contents[$key]['row_tax'] = $this->erp->formatDecimal(($this->_cart_contents[$key]['tax'] * $this->_cart_contents[$key]['qty']), 4);
            $this->_cart_contents[$key]['subtotal'] = $this->erp->formatDecimal(($this->_cart_contents[$key]['price'] * $this->_cart_contents[$key]['qty']), 4);
        }
    ...
    public function total() {
        return $this->erp->formatDecimal($this->_cart_contents['cart_total'], 4);
    }

    public function shipping() {
        return $this->erp->formatDecimal($this->shop_settings->shipping, 4);
    }
        ...
        if (!empty($this->Settings->tax2)) {
            if ($order_tax_details = $this->site->getTaxRateByID($this->Settings->default_tax_rate2)) {
                if ($order_tax_details->type == 2 || $order_tax_details->rate == 0) {
                    $order_tax = $this->erp->formatDecimal($order_tax_details->rate, 4);
                } elseif ($order_tax_details->type == 1) {
                    $order_tax = $this->erp->formatDecimal(((($this->total()) * $order_tax_details->rate) / 100), 4);
                }
                return $order_tax;
            }
        }
    ...
    public function total_item_tax() {
        return $this->erp->formatDecimal($this->_cart_contents['total_item_tax'], 4);
    }
    ...
    function cart_data($re = false) {
        ...
        foreach($citems as &$value) {
            $value['price'] = $this->erp->convertMoney($value['price']);
            $value['subtotal'] = $this->erp->convertMoney($value['subtotal']);
            if ($this->has_options($value['rowid'])) {
                $value['options'] = $this->product_options($value['rowid']);
                foreach($value['options'] as &$opt_value) {
                    $opt_value['price'] = $this->erp->convertMoney($opt_value['price']);
                }
            }
        }
        $total = $this->erp->convertMoney($this->total(), FALSE, FALSE);
        $shipping = $this->erp->convertMoney($this->shipping(), FALSE, FALSE);
        $order_tax = $this->erp->convertMoney($this->order_tax(), FALSE, FALSE);
        $cart = array(
            'total_items' => $this->total_items(),
            'total_unique_items' => $this->total_items(TRUE),
            'contents' => $citems,
            'total_item_tax' => $this->erp->convertMoney($this->total_item_tax()),
            'subtotal' => $this->erp->convertMoney($this->total()-$this->total_item_tax()),
            'total' => $this->erp->formatMoney($total, $this->selected_currency->symbol),
            'shipping' => $this->erp->formatMoney($shipping, $this->selected_currency->symbol),
            'order_tax' => $this->erp->formatMoney($order_tax, $this->selected_currency->symbol),
            'grand_total' => $this->erp->formatMoney(($this->erp->formatDecimal($total)+$this->erp->formatDecimal($order_tax)+$this->erp->formatDecimal($shipping)), $this->selected_currency->symbol),
            );
