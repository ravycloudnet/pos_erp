...
class Gst
{
    ...
    function genHTML($tax_summary = [], $product_tax = 0) {
        ...
        if (!empty($tax_summary)) {
            ...
            foreach ($tax_summary as $summary) {
                $html .= '<tr><td>' . $summary['name'] . '</td><td style="text-align:center;">' . $summary['code'] . '</td><td class="text-center">' . $this->erp->formatQuantity($summary['items']) . '</td><td style="text-align:right;">' . $this->erp->formatMoney($summary['amt']) . '</td><td class="text-right">' . $this->erp->formatMoney($summary['tax']) . '</td></tr>';
            }
            $html .= '</tbody></tfoot>';
            $html .= '<tr class="active"><th colspan="4" style="text-align:right;">' . lang('total_tax_amount') . '</th><th class="text-right">' . $this->erp->formatMoney($product_tax) . '</th></tr>';
            $html .= '</tfoot></table>';
        }
        return $html;
    }
    ...
    function calculteIndianGST($item_tax, $state, $tax_details) {
        if ($this->Settings->indian_gst) {
            $cgst = $sgst = $igst = 0;
            if ($state) {
                $gst = $tax_details->type == 1 ? $this->erp->formatDecimal(($tax_details->rate/2), 0).'%' : $this->erp->formatDecimal(($tax_details->rate/2), 0);
                $cgst = $this->erp->formatDecimal(($item_tax / 2), 4);
                $sgst = $this->erp->formatDecimal(($item_tax / 2), 4);
            } else {
                $gst = $tax_details->type == 1 ? $this->erp->formatDecimal(($tax_details->rate), 0).'%' : $this->erp->formatDecimal(($tax_details->rate), 0);
                $igst = $item_tax;
            }
