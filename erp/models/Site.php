...
class Site extends CI_Model
{
        ...
        if ($product = $this->getProductByID($clause['product_id'])) {
            if ($pi = $this->getPurchasedItem($clause)) {
                ...
                if (isset($product->tax_rate) && $product->tax_rate != 0) {
                    ...
                    $pr_item_tax = $this->erp->formatDecimal($item_tax * $clause['unit_quantity'], 4);
    ...
    public function syncSalePayments($id) {
        ...
        if ($payments = $this->getSalePayments($id)) {
            ...
            if ($this->erp->formatDecimal($grand_total) == $this->erp->formatDecimal($paid)) {
                $payment_status = 'paid';
    ...
    function calculateTax($product_details = NULL, $tax_details, $custom_value = NULL, $c_on = NULL) {
        ...
        if ($tax_details && $tax_details->type == 1 && $tax_details->rate != 0) {
            if ($product_details && $product_details->tax_method == 1) {
                $tax_amount = $this->erp->formatDecimal((($value) * $tax_details->rate) / 100, 4);
                $tax = $this->erp->formatDecimal($tax_details->rate, 0) . "%";
            } else {
                $tax_amount = $this->erp->formatDecimal((($value) * $tax_details->rate) / (100 + $tax_details->rate), 4);
                $tax = $this->erp->formatDecimal($tax_details->rate, 0) . "%";
            }
        } elseif ($tax_details && $tax_details->type == 2) {
            $tax_amount = $this->erp->formatDecimal($tax_details->rate);
            $tax = $this->erp->formatDecimal($tax_details->rate, 0);
        }
    ...
    public function calculateDiscount($discount = NULL, $amount) {
        if ($discount && $this->Settings->product_discount) {
            ...
            if ($dpos !== false) {
                ...
                return $this->erp->formatDecimal(((($this->erp->formatDecimal($amount)) * (Float) ($pds[0])) / 100), 4);
            } else {
                return $this->erp->formatDecimal($discount, 4);
            }
        }
        return 0;
    }
        ...
        if ($this->Settings->tax2 != 0 && $order_tax_id) {
            if ($order_tax_details = $this->site->getTaxRateByID($order_tax_id)) {
                if ($order_tax_details->type == 1) {
                    return $this->erp->formatDecimal((($amount * $order_tax_details->rate) / 100), 4);
                } else {
                    return $this->erp->formatDecimal($order_tax_details->rate, 4);
                }
            }
        }
        return 0;
