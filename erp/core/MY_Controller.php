...
class MY_Controller extends CI_Controller {
    ...
    function __construct()
    {
        ...
        if($erp_language = $this->input->cookie('erp_language', TRUE)) {
            $this->config->set_item('language', $erp_language);
            $this->lang->admin_load('erp', $erp_language);
            $this->Settings->user_language = $erp_language;
        } else {
            $this->config->set_item('language', $this->Settings->language);
            $this->lang->admin_load('erp', $this->Settings->language);
            $this->Settings->user_language = $this->Settings->language;
        }
        if($rtl_support = $this->input->cookie('erp_rtl_support', TRUE)) {
