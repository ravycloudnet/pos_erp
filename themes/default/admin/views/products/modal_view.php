                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="width:70%;">
                                        ...
                                        <?= $this->erp->qrcode('link', urlencode(admin_url('products/view/' . $product->id)), 2); ?>
                                    </td>
                                </tr>
                                <tr>
                                    <td><?= lang("type"); ?></td>
                                ...
                                <?php if ($product->subcategory_id) { ?>
                                    ...
                                    <?php if ($Owner || $Admin) {
                                        echo '<tr><td>' . lang("cost") . '</td><td>' . $this->erp->formatMoney($product->cost) . '</td></tr>';
                                        echo '<tr><td>' . lang("price") . '</td><td>' . $this->erp->formatMoney($product->price) . '</td></tr>';
                                        if ($product->promotion) {
                                            echo '<tr><td>' . lang("promotion") . '</td><td>' . $this->erp->formatMoney($product->promo_price) . ' ('.$this->erp->hrsd($product->start_date).' - '.$this->erp->hrsd($product->end_date).')</td></tr>';
                                        }
                                    } else {
                                        ...
                                        if ($this->session->userdata('show_price')) {
                                            echo '<tr><td>' . lang("price") . '</td><td>' . $this->erp->formatMoney($product->price) . '</td></tr>';
                                            if ($product->promotion) {
                                                echo '<tr><td>' . lang("promotion") . '</td><td>' . $this->erp->formatMoney($product->promo_price) . ' ('.$this->erp->hrsd($product->start_date).' - '.$this->erp->hrsd($product->start_date).')</td></tr>';
                                            }
                                        }
                                    }
                                    ?>
                            ...
                            foreach ($options as $option) {
                                if ($option->wh_qty != 0) {
                                    ...
                                    if ($Owner || $Admin && (!$Customer || $this->session->userdata('show_cost'))) {
                                        echo '<td class="text-right">' . $this->erp->formatMoney($option->price) . '</td>';
                                    }
                                    echo '</tr>';
                                }
