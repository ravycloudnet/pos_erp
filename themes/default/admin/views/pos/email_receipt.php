                    ...
                    <tr>
                        <td>
                            ...
                            <div style="text-align:center;">
                                <img src="<?= admin_url('misc/barcode/'.$this->erp->base64url_encode($inv->reference_no).'/code128/74/0/1'); ?>" alt="<?= $inv->reference_no; ?>" class="bcimg" />
                                <?= $this->erp->qrcode('link', urlencode(admin_url('pos/view/' . $inv->id)), 2); ?>
                            </div>
                            <div style="clear:both;height:15px;"></div>
                                <div id="receiptData" style="border:1px solid #DDD; padding:10px; text-align:center;">
                                        ...
                                        echo "<p>" .lang("date") . ": " . $this->erp->hrld($inv->date) . "<br>";
                                        ...
                                        <table width="100%" style="margin:15px 0;">
                                            <tbody>
                                                ...
                                                foreach ($rows as $row) {
                                                    ...
                                                    echo '<tr><td style="text-align:left;border-bottom:1px solid #DDD;">' . $this->erp->formatQuantity($row->unit_quantity) . ' x '.$this->erp->formatMoney($row->unit_price).($row->item_tax != 0 ? ' - '.lang('tax').' <small>('.($Settings->indian_gst ? $row->tax : $row->tax_code).')</small> '.$this->erp->formatMoney($row->item_tax).' ('.lang('hsn_code').': '.$row->hsn_code.')' : '').'</td><td style="text-align:right;border-bottom:1px solid #DDD;">' . $this->erp->formatMoney($row->subtotal) . '</td></tr>';
                                            ...
                                            <tfoot>
                                                <tr>
                                                    ...
                                                    <th style="text-align:right;border-bottom:1px solid #DDD;"><?=$this->erp->formatMoney($return_sale ? (($inv->total + $inv->product_tax)+($return_sale->total + $return_sale->product_tax)) : ($inv->total + $inv->product_tax));?></th>
                                                </tr>
                                                <?php
                                                if ($inv->order_tax != 0) {
                                                    echo '<tr><th style="text-align:left;border-bottom:1px solid #DDD;">' . lang("tax") . '</th><th style="text-align:right;border-bottom:1px solid #DDD;">' . $this->erp->formatMoney($return_sale ? ($inv->order_tax+$return_sale->order_tax) : $inv->order_tax) . '</th></tr>';
                                                }
                                                if ($inv->order_discount != 0) {
                                                    echo '<tr><th style="text-align:left;border-bottom:1px solid #DDD;">' . lang("order_discount") . '</th><th style="text-align:right;border-bottom:1px solid #DDD;">' . $this->erp->formatMoney($inv->order_discount) . '</th></tr>';
                                                }

                                                if ($inv->shipping != 0) {
                                                    echo '<tr><th style="text-align:left;border-bottom:1px solid #DDD;">' . lang("shipping") . '</th><th style="text-align:right;border-bottom:1px solid #DDD;">' . $this->erp->formatMoney($inv->shipping) . '</th></tr>';
                                                }

                                                if ($return_sale) {
                                                    if ($return_sale->surcharge != 0) {
                                                        echo '<tr><th style="text-align:left;border-bottom:1px solid #DDD;">' . lang("order_discount") . '</th><th style="text-align:right;border-bottom:1px solid #DDD;">' . $this->erp->formatMoney($return_sale->surcharge) . '</th></tr>';
                                                    }
                                                }
                                                ...
                                                if ($Settings->indian_gst) {
                                                    if ($inv->cgst > 0) {
                                                        ...
                                                        echo '<tr><td>' . lang('cgst') .'</td><td style="text-align:right;border-bottom:1px solid #DDD;">' . ( $Settings->format_gst ? $this->erp->formatMoney($cgst) : $cgst) . '</td></tr>';
                                                    }
                                                    if ($inv->sgst > 0) {
                                                        $sgst = $return_sale ? $inv->sgst + $return_sale->sgst : $inv->sgst;
                                                        echo '<tr><td>' . lang('sgst') .'</td><td style="text-align:right;border-bottom:1px solid #DDD;">' . ( $Settings->format_gst ? $this->erp->formatMoney($sgst) : $sgst) . '</td></tr>';
                                                    }
                                                    if ($inv->igst > 0) {
                                                        $igst = $return_sale ? $inv->igst + $return_sale->igst : $inv->igst;
                                                        echo '<tr><td>' . lang('igst') .'</td><td style="text-align:right;border-bottom:1px solid #DDD;">' . ( $Settings->format_gst ? $this->erp->formatMoney($igst) : $igst) . '</td></tr>';
                                                    }
                                                }
                                                ...
                                                if ($pos_settings->rounding || $inv->rounding != 0) {
                                                    ...
                                                    <tr>
                                                        ...
                                                        <th style="text-align:right;border-bottom:1px solid #DDD;"><?= $this->erp->formatMoney($inv->rounding);?></th>
                                                    </tr>
                                                    <tr>
                                                        <th style="text-align:left;border-bottom:1px solid #DDD;"><?=lang("grand_total");?></th>
                                                        <th style="text-align:right;border-bottom:1px solid #DDD;"><?=$this->erp->formatMoney($return_sale ? (($inv->grand_total + $inv->rounding)+$return_sale->grand_total) : ($inv->grand_total + $inv->rounding));?></th>
                                                    </tr>
                                                    <?php
                                                } else {
                                                    ?>
                                                    <tr>
                                                        <th style="text-align:left;border-bottom:1px solid #DDD;"><?=lang("grand_total");?></th>
                                                        <th style="text-align:right;border-bottom:1px solid #DDD;"><?=$this->erp->formatMoney($return_sale ? ($inv->grand_total+$return_sale->grand_total) : $inv->grand_total);?></th>
                                                    </tr>
                                                    <?php
                                                }
                                                if ($inv->paid < ($inv->grand_total + $inv->rounding)) {
                                                    ...
                                                    <tr>
                                                        ...
                                                        <th style="text-align:right;border-bottom:1px solid #DDD;"><?=$this->erp->formatMoney($return_sale ? ($inv->paid+$return_sale->paid) : $inv->paid);?></th>
                                                    </tr>
                                                    <tr>
                                                        <th style="text-align:left;border-bottom:1px solid #DDD;"><?=lang("due_amount");?></th>
                                                        <th style="text-align:right;border-bottom:1px solid #DDD;"><?=$this->erp->formatMoney(($return_sale ? (($inv->grand_total + $inv->rounding)+$return_sale->grand_total) : ($inv->grand_total + $inv->rounding)) - ($return_sale ? ($inv->paid+$return_sale->paid) : $inv->paid));?></th>
                                                    </tr>
                                                    <?php
                                                } ?>
                                            </tfoot>
                                        ...
                                        if ($payments) {
                                            ...
                                            foreach ($payments as $payment) {
                                                ...
                                                if (($payment->paid_by == 'cash' || $payment->paid_by == 'deposit') && $payment->pos_paid) {
                                                    ...
                                                    echo '<td style="text-align:left;border-bottom:1px solid #DDD;">' . lang("amount") . ': ' . $this->erp->formatMoney($payment->pos_paid == 0 ? $payment->amount : $payment->pos_paid) . ($payment->return_id ? ' (' . lang('returned') . ')' : '') . '</td>';
                                                    echo '<td style="text-align:left;border-bottom:1px solid #DDD;">' . lang("change") . ': ' . ($payment->pos_balance > 0 ? $this->erp->formatMoney($payment->pos_balance) : 0) . '</td>';
                                                } elseif (($payment->paid_by == 'CC' || $payment->paid_by == 'ppp' || $payment->paid_by == 'stripe') && $payment->cc_no) {
                                                    echo '<td style="text-align:left;border-bottom:1px solid #DDD;">' . lang("paid_by") . ': ' . lang($payment->paid_by) . '</td>';
                                                    echo '<td style="text-align:left;border-bottom:1px solid #DDD;">' . lang("amount") . ': ' . $this->erp->formatMoney($payment->pos_paid) . ($payment->return_id ? ' (' . lang('returned') . ')' : '') . '</td>';
                                                    ...
                                                    echo '<td style="text-align:left;border-bottom:1px solid #DDD;">' . lang("amount") . ': ' . $this->erp->formatMoney($payment->pos_paid) . ($payment->return_id ? ' (' . lang('returned') . ')' : '') . '</td>';
                                                    echo '<td style="text-align:left;border-bottom:1px solid #DDD;">' . lang("balance") . ': ' . ($payment->pos_balance > 0 ? $this->erp->formatMoney($payment->pos_balance) : 0) . '</td>';
                                        ...
                                        <?= $biller->invoice_footer ? '<p class="text-center">'.$this->erp->decode_html($biller->invoice_footer).'</p>' : ''; ?>
                                        </div>
                                    </div>
