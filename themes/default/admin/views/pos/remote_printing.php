...
if ($pos_settings->remote_printing != 1) {
    ...
    <script type="text/javascript">
        ...
        function receiptData() {
            ...
            receipt.info = "";
            receipt.info += "<?= lang("date") . ": " . $this->erp->hrld($inv->date); ?>" + "\n";
            ...
            <?php if ($row->item_tax != 0) { ?>
                receipt.items += "<?= '    '.lang('tax').' ('.($Settings->indian_gst ? $row->tax : $row->tax_code).') '.$this->erp->formatMoney($row->item_tax).' ('.lang('hsn_code').': '.$row->hsn_code.')'; ?>" + "\n";
            <?php } ?>
            receipt.items += "<?= printLine("   ".($this->erp->formatQuantity($row->unit_quantity).' '.$row->product_unit_code)." x ".$this->erp->formatMoney($row->unit_price) . ":  ". $this->erp->formatMoney($row->subtotal), $char_per_line, ' '); ?>" + "\n";
            <?php $r++; endforeach; ?>
            <?php
            if ($return_rows) { ?>
                receipt.items += "\n" + "<?=lang('returned_items');?>" + "\n";
            ...
            receipt.totals = "";
            receipt.totals += "<?= printLine(lang("total") . ": " . $this->erp->formatMoney($return_sale ? (($inv->total + $inv->product_tax)+($return_sale->total + $return_sale->product_tax)) : ($inv->total + $inv->product_tax)), $char_per_line); ?>" + "\n";
            <?php
            if ($Settings->indian_gst) {
                if ($inv->cgst > 0) {
                    $cgst = $return_sale ? $inv->cgst + $return_sale->cgst : $inv->cgst; ?>
                    receipt.totals += "<?= printLine(lang("cgst") . ": " . ( $Settings->format_gst ? $this->erp->formatMoney($cgst) : $cgst), $char_per_line); ?>" + "\n";
                    <?php
                }
                if ($inv->sgst > 0) {
                    $sgst = $return_sale ? $inv->sgst + $return_sale->sgst : $inv->sgst; ?>
                    receipt.totals += "<?= printLine(lang("sgst") . ": " . ( $Settings->format_gst ? $this->erp->formatMoney($sgst) : $sgst), $char_per_line); ?>" + "\n";
                    <?php
                }
                if ($inv->igst > 0) {
                    $igst = $return_sale ? $inv->igst + $return_sale->igst : $inv->igst; ?>
                    receipt.totals += "<?= printLine(lang("igst") . ": " . ( $Settings->format_gst ? $this->erp->formatMoney($igst) : $igst), $char_per_line); ?>" + "\n";
                    <?php
                }
            }

            if ($inv->order_tax != 0) { ?>
                receipt.totals += "<?= printLine(lang("tax") . ": " . $this->erp->formatMoney($return_sale ? ($inv->order_tax+$return_sale->order_tax) : $inv->order_tax), $char_per_line); ?>" + "\n";
                <?php
            }
            if ($inv->total_discount != 0) { ?>
                receipt.totals += "<?= printLine(lang("discount") . ": (" . $this->erp->formatMoney($return_sale ? ($inv->product_discount+$return_sale->product_discount) : $inv->product_discount) . ") " . $this->erp->formatMoney($return_sale ? ($inv->order_discount+$return_sale->order_discount) : $inv->order_discount), $char_per_line); ?>" + "\n";
                <?php
            }
            if ($inv->shipping != 0) { ?>
                receipt.totals += "<?= printLine(lang("shipping") . ": ". $this->erp->formatMoney($inv->shipping), $char_per_line); ?>" + "\n";
                <?php
            }
            if ($pos_settings->rounding || $inv->rounding != 0) { ?>
                receipt.totals += "<?= printLine(lang("rounding") . ": " . $this->erp->formatMoney($inv->rounding), $char_per_line); ?>" + "\n";
                receipt.totals += "<?= printLine(lang("grand_total") . ": " . $this->erp->formatMoney($return_sale ? ($this->erp->roundMoney($inv->grand_total + $inv->rounding)+$return_sale->grand_total) : $this->erp->roundMoney($inv->grand_total + $inv->rounding)), $char_per_line); ?>" + "\n";
                <?php
            } else { ?>
                receipt.totals += "<?= printLine(lang("grand_total") . ": " . $this->erp->formatMoney($return_sale ? ($inv->grand_total+$return_sale->grand_total) : $inv->grand_total), $char_per_line); ?>" + "\n";
                <?php
            } ?>
            receipt.totals += "<?= printLine(lang("paid_amount") . ": " . $this->erp->formatMoney($return_sale ? ($inv->paid+$return_sale->paid) : $inv->paid), $char_per_line); ?>" + "\n";
            receipt.totals += "<?= printLine(lang("due_amount") . ": " . $this->erp->formatMoney(($return_sale ? ($inv->grand_total+$return_sale->grand_total) : $inv->grand_total) - ($return_sale ? ($inv->paid+$return_sale->paid) : $inv->paid)), $char_per_line); ?>" + "\n";
            ...
            if($payments) {
                ...
                foreach($payments as $payment) {
                    if ($payment->paid_by == 'cash'  || $payment->paid_by == 'deposit' && $payment->pos_paid) { ?>
                        ...
                        receipt.payments += "<?= printLine(lang("amount") . ": " . $this->erp->formatMoney($payment->pos_paid), $char_per_line); ?>" + "\n";
                        receipt.payments += "<?= printLine(lang("change") . ": " . ($payment->pos_balance > 0 ? $this->erp->formatMoney($payment->pos_balance) : '0.00'), $char_per_line); ?>" + "\n";
                        <?php
                        ...
                        <?php if ($payment->paid_by == 'other' && $payment->amount) { ?>
                            receipt.payments += "<?= printLine(lang("paid_by") . ": " . lang($payment->paid_by), $char_per_line); ?>" + "\n";
                            receipt.payments += "<?= printLine(lang("amount") . ": " . $this->erp->formatMoney($payment->amount), $char_per_line); ?>" + "\n";
            ...
            if ($Settings->invoice_view > 0) {
                if (!empty($tax_summary)) {
                    ...
                    receipt.footer += "<?=taxLine($summary['name'], $summary['code'], $this->erp->formatQuantity($summary['items']), $this->erp->formatMoney($summary['amt']), $this->erp->formatMoney($summary['tax']), $char_per_line);?>" + "\n";
                    <?php endforeach;?>
                    receipt.footer += "<?=str_replace("\n", "", drawLine($char_per_line));?>"; + "\n";
                    receipt.footer += "\n<?=printLine(lang("total_tax_amount") . ":" . $this->erp->formatMoney($inv->product_tax), $char_per_line);?>" + "\n";
                    receipt.footer += "<?=str_replace("\n", "", drawLine($char_per_line));?>" + "\n\n";
                    <?php
                }
            }
            if ($inv->note) { ?>
                receipt.footer += "<?= printText(strip_tags(preg_replace('/\s+/',' ', $this->erp->decode_html($inv->note))), $char_per_line); ?>" + "\n\n";
                <?php
            }
