        ...
        <div class="well well-sm">
            <div class="row bold">
                ...
                <div class="col-xs-6 pull-right text-right order_barcodes">
                    <?php
                    $path = admin_url('misc/barcode/'.$this->erp->base64url_encode($transfer->transfer_no).'/code128/74/0/1');
                    $type = $Settings->barcode_img ? 'png' : 'svg+xml';
                    $data = file_get_contents($path);
                    $base64 = 'data:image/' . $type . ';base64,' . base64_encode($data);
                    ?>
                    <img src="<?= $base64; ?>" alt="<?= $transfer->transfer_no; ?>" class="bcimg" />
                    <?php /*echo $this->erp->qrcode('link', urlencode(admin_url('transfers/view/' . $transfer->id)), 2);*/ ?>
                </div>
                ...
                <table class="table table-bordered table-hover table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <?php } ?>
                            <td style="text-align:center;width:80px; "><?php echo $this->erp->formatQuantity($row->unit_quantity).' '.$row->product_unit_code; ?></td>
                            <td style="width:90px;text-align:right;vertical-align:middle;"><?php echo $this->erp->formatMoney($row->net_unit_cost); ?></td>
                            <?php
                            if ($Settings->tax1) {
                                echo '<td style="width: 80px; text-align:right; vertical-align:middle;">' . ($Settings->indian_gst ? '<small>('.$row->tax.')</small>' : '') . ' ' . $this->erp->formatMoney($row->item_tax) . '</td>';
                            }
                            ?>
                            <td style="width:100px;text-align:right;vertical-align:middle;"><?php echo $this->erp->formatMoney($row->subtotal); ?></td>
                        </tr>
                        <?php $r++;
                        endforeach;
                        ?>
                    ...
                    <tfoot>
                        ...
                        <tr>
                            ...
                            <td style="text-align:right;"><?= $this->erp->formatMoney($transfer->total_tax); ?></td>
                            <td style="text-align:right;"><?= $this->erp->formatMoney($transfer->total+$transfer->total_tax); ?></td>
                        </tr>
                        <?php if ($Settings->indian_gst) {
                            if ($transfer->cgst > 0) {
                                echo '<tr><td colspan="' . ($col+1) . '" class="text-right">' . lang('cgst') . ' (' . $default_currency->code . ')</td><td class="text-right">' . ( $Settings->format_gst ? $this->erp->formatMoney($transfer->cgst) : $transfer->cgst) . '</td></tr>';
                            }
                            if ($transfer->sgst > 0) {
                                echo '<tr><td colspan="' . ($col+1) . '" class="text-right">' . lang('sgst') . ' (' . $default_currency->code . ')</td><td class="text-right">' . ( $Settings->format_gst ? $this->erp->formatMoney($transfer->sgst) : $transfer->sgst) . '</td></tr>';
                            }
                            if ($transfer->igst > 0) {
                                echo '<tr><td colspan="' . ($col+1) . '" class="text-right">' . lang('igst') . ' (' . $default_currency->code . ')</td><td class="text-right">' . ( $Settings->format_gst ? $this->erp->formatMoney($transfer->igst) : $transfer->igst) . '</td></tr>';
                            }
                        } ?>
                        <tr>
                            <td colspan="<?= $col+1; ?>"
                            ...
                            <td style="text-align:right; font-weight:bold;"><?= $this->erp->formatMoney($transfer->grand_total); ?></td>
                        </tr>
                    </tfoot>
                </table>
            </div>
