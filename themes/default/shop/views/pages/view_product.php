                                        ...
                                        <div class="form-group">
                                            ...
                                            if ($variants) {
                                                foreach ($variants as $variant) {
                                                    $opts[$variant->id] = $variant->name.($variant->price > 0 ? ' (+'.$this->erp->convertMoney($variant->price, TRUE, FALSE).')' : ($variant->price == 0 ? '' : ' (+'.$this->erp->convertMoney($variant->price, TRUE, FALSE).')'));
                                                }
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-bordered table-striped dfTable table-right-left">
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        <td><?= lang("price"); ?></td>
                                                        <td><?= $this->erp->convertMoney($product->special_price ? $product->special_price : $product->price); ?></td>
                                                    </tr>
                                                    <?php } ?>

                                                    <?php
                                                    if ($product->promotion) {
                                                        echo '<tr><td>' . lang("promotion") . '</td><td>' . $this->erp->convertMoney($product->promo_price) . ' ('.$this->erp->hrsd($product->start_date).' - '.$this->erp->hrsd($product->start_date).')</td></tr>';
                                                    }
                                                    ?>
                                                    ...
                                                    <tr>
                                                        <td><?= lang("in_stock"); ?></td>
                                                        <td><?= $this->erp->formatQuantity($warehouse->quantity); ?></td>
                                                    </tr>
                                                    <?php } ?>
                                                    ...
                                                    <?php if (!empty($options)) {
                                                        foreach ($options as $option) {
                                                            if ($option->wh_qty != 0) {
                                                                ...
                                                                echo '<td>' .lang("in_stock").': '. $this->erp->formatQuantity($option->wh_qty) . '</td>';
                                                                echo '<td>' .lang("price").': '.$this->erp->convertMoney(($product->special_price ? $product->special_price : $product->price)+$option->price). '</td>';
                                                                echo '</tr>';
                                                            }
                            ...
                            <div class="details" style="transition: all 100ms ease-out 0s;">
                                ...
                                <div class="stats-container">
                                    ...
                                    <span class="product_price">
                                        <?php
                                        if ($fp->promotion) {
                                            echo '<del class="text-red">'.$this->erp->convertMoney(isset($fp->special_price) && !empty($fp->special_price) ? $fp->special_price : $fp->price).'</del><br>';
                                            echo $this->erp->convertMoney($fp->promo_price);
                                        } else {
                                            echo $this->erp->convertMoney(isset($fp->special_price) && !empty($fp->special_price) ? $fp->special_price : $fp->price);
                                        }
                                        ?>
                                    </span>
                                    <?php } ?>
