                                    ...
                                    foreach ($orders as $order) {
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <table class="table table-borderless table-condensed" style="margin-bottom:0;">
                                                <?= '<tr><td>'.lang('date').'</td><td>'.$this->erp->hrld($order->date).'</td></tr>'; ?>
                                                <?= '<tr><td>'.lang('ref').'</td><td>'.$order->reference_no.'</td></tr>'; ?>
                                                <?= '<tr><td>'.lang('status').'</td><td>'.$order->status.'</td></tr>'; ?>
                                                <?= '<tr><td>'.lang('amount').'</td><td>'.$this->erp->formatMoney($order->grand_total, $this->default_currency->symbol).'</td></tr>'; ?>
                                                <?= '<tr><td>'.lang('comment').'</td><td>'.$this->erp->decode_html($order->note).'</td></tr>'; ?>
                                                </table>
